...
/*checking for duplicates before Data Exploration*/
...
/*Total Trips*/
SELECT COUNT(*) as total_trips FROM trips_details4
WHERE end_ride=1;
...
select sum(end_ride) as total_trips from trips_details4;
...
select avg(fare) as Average_fair from trips;
...
from trips
group by faremethod
order by paycnt desc 
OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY) mostused
...
--Which top 2 locations had the most trips
Select * 
From
(
...
;
-------------OR------------------------
SELECT loc_from.assembly1 AS loc_from_assembly,
       loc_to.assembly1 AS loc_to_assembly,
       b.*
FROM (
    SELECT *, DENSE_RANK() OVER (ORDER BY trip DESC) AS rnk
    FROM (
        SELECT t.loc_from, t.loc_to, COUNT(DISTINCT t.tripid) AS trip
        FROM trips t
        GROUP BY t.loc_from, t.loc_to
    ) a
) b
JOIN loc loc_from ON b.loc_from = loc_from.id
JOIN loc loc_to ON b.loc_to = loc_to.id
WHERE b.rnk=1;
...
(select top 1 duration, count(distinct tripid) cnt 
from trips
...
select sum(searches_got_estimate)*100.0/sum(searches)as search_to_Estimate
from trips_details4;
...
select sum(searches_got_quotes)*100.0/sum(searches_for_quotes)
from trips_details4;
...
select sum(driver_not_cancelled)*100.0/sum(customer_not_cancelled)
from trips_details4;
...
select top 1 loc_from,sum(fare) as total_fare from trips
group by loc_from
...
--Driver Cancellation location-wise
select * from
...
--Customer Cancellation location wise
select * from
(select *, rank() over (order by cancelled desc) rnk
from
...
from
( select duration,count(distinct tripid) trip
from trips
group by duration)b)c
where rnk=1;